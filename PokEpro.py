...
_ALL_TYPES = sorted(set(list(_TYPE_CHART.keys()) + [d for a in _TYPE_CHART for d in _TYPE_CHART[a].keys()]))


def _attack_multiplier(attack_type, defend_types):
    """Return the damage multiplier when attack_type hits a Pokémon with defend_types (list)."""
...
def compute_strengths_and_weaknesses(types):
    """Given a Pokémon's typing (list), return (strength_list, weakness_list).

    - strength_list: defender types where this Pokémon's attacks are super-effective (>1)
    - weakness_list: attacking types that are super-effective against this Pokémon (>1)
    """
    ...
    # Strengths: for each possible defender type, check whether any of this Pokémon's own attack types
    # would be super-effective against that defender.
    ...
    # Weaknesses: incoming attack types that are super-effective against this Pokémon's typing
...
# Replace the previous large inline dict with the loader (existing code kept, shown here)
...
if pokemon_by_number:
    ...
    print("No Pokédex data loaded (pokemon_by_number is empty). Add a valid pokemon_data.txt in the same folder.", file=sys.stderr)
